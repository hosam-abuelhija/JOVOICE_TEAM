...
namespace JOVOICE.Controllers
{
    public class TempAdsHadeelController : Controller
    {
        ...
        public ActionResult Create1([Bind(Include = "id,name,listname,electionarea,description")] TempAd tempAd, HttpPostedFileBase upload)
        {
            ...
            //    var path = Path.Combine(Server.MapPath("~/Content/Adsimg/"), uniqueFileName);

            //    imageFile.SaveAs(path);

            //    // Delete the old image file if it exists
            //    if (!string.IsNullOrEmpty(tempAd.image))
            //    {
            //        var oldImagePath = Path.Combine(Server.MapPath("~/Images"), tempAd.image);
